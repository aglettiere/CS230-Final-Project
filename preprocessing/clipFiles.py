...
for dance in ["ballet", "cha", "pasodoble", "tap", "jive", "rumba", "break", "flamenco", "foxtrot", "latin", "quickstep", "square", "swing", "tango", "waltz"]:
#	if dance not in ["cha", "pasodoble", "tap", "jive", "rumba", "ballet", "break", "flamenco", "foxtrot", "latin", "quickstep", "square", "swing", "tango", "waltz"]:
	...
	os.system("mkdir ./moco/" + dance)
	video_names = [f[0:-8] for f in os.listdir(os.path.join(rootdir, dance))]
	video_names = list(set(video_names))
	for video in video_names:
		os.system("mkdir ./moco/" + dance + "/" + video)
		for i in range(1,33):
			numZeros = 4-len(str(i))
			zeros = "0"*numZeros
			os.system("cp " + os.path.join(rootdir, dance) + "/" + video + zeros + str(i) + ".jpg ./moco/" + dance + "/" + video + "/")
